"""
Generates Final/Report/FinalProject_Arsov_Outline.docx, the concise intermediate submission.
...
body(
    ...
    'and GPU (CUDA) for two questions: (1) raw throughput vs batch size and tpb '
    'configuration, and (2) truncated collision search over bit widths 16 to 72. '
    'GPU achieves 167x to 440x throughput speedup and up to 260x collision-search '
...
body(
    ...
    'truncated collision search, which is directly relevant to practical '
...
bullet('Task 3: Truncated collision search (CPU serial vs OpenMP vs GPU Pollard\'s rho), bits in {16..72}')
...
bullet('task2.cu: same kernel, sweeps tpb 32 to 1024')
...
bullet('CPU_MAX_BITS=56 with --mem=96G; bits=64+ are GPU-only (CPU needs ~100 GB for unordered_map at bits=64)')
...
body('GPU breaks even with CPU serial at n ≈ 200,000 (kernel launch overhead of about 2 ms dominates below this).')
...
body('tpb=32 is optimal for all algorithms. Cause: register pressure limits SM occupancy. Larger blocks claim more of the 65K-register file, reducing concurrent warps. MD5 is most sensitive (-27%), SHA-256 least (-6%).')
...
add_table(
    ...
    [
        ...
        ['MD5',     '64', 'n/a',     'n/a',     '8,488', '(CPU infeasible)'],
        ['SHA-1',   '56', '74,417',  '35,765',  '2,375', '31x'],
        ['SHA-256', '56', '316,526', '181,146', '1,930', '164x'],
    ]
)
bullet('OMP speedup over CPU serial: only 1.5x to 2.1x at bits=56. Serial merge phase (Amdahl) and NUMA dominate.')
bullet('12-thread OMP sweet spot confirmed by dedicated scaling experiment (sweep 1 to 32 threads): NUMA boundary at socket edge, L3 cache saturation, Amdahl ceiling.')
bullet('GPU is the only feasible approach at bits >= 64. CPU birthday attack needs roughly 100 GB for unordered_map; GPU Pollard\'s rho uses 8 MB.')
bullet('Birthday paradox validation: cpu_count divided by expected is roughly 1.0 across all algorithms and bit widths, confirming correct truncated hash extraction.')
...
bullet(
    'Thrust Parallel Sort (thrust::sort_by_key plus adjacent scan): '
    'fully GPU-resident, cache-friendly radix sort; '
    'expected fastest at small bits, crossover with Pollard\'s rho around bits=52 to 56 as working set fills VRAM.'
)
bullet(
...
# 4. Deliverables
h1('4. Deliverables')
bullet('Final/Code/: task1.cu, task2.cu, task3.cu, plus CPU hash implementations')
...
bullet('GPU throughput advantage (167x to 440x) comes from embarrassingly parallel independent hashes. tpb=32 is optimal due to register pressure, giving a free 7% to 33% gain over the tpb=256 default.')
...
bullet('ME759 concepts applied: CUDA template kernels, occupancy and register analysis, OpenMP parallel-for, NUMA topology, Amdahl\'s Law, cudaEvent timing.')

h1('References')
body('[1] Rivest (1992). RFC 1321: MD5 Message-Digest Algorithm.')
body('[2] NIST FIPS 180-4: Secure Hash Standard. 2015.')
body('[3] van Oorschot and Wiener (1999). Parallel Collision Search with Cryptanalytic Applications. J. Cryptology 12(1).')
body('[4] Kirk and Hwu (2016). Programming Massively Parallel Processors, 3rd ed.')
...
body('[7] Flajolet, P., and Odlyzko, A. (1990). Random mapping statistics. '
     'EUROCRYPT \'89, LNCS vol. 434, pp. 329-354. Springer. '
     '(Birthday paradox collision expectation formula.)')
body('[8] Bell, N., and Hoberock, J. (2011). Thrust: A productivity-oriented library '
